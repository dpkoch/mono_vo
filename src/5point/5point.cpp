...
bool Solve5PointEssential(const std::vector<cv::KeyPoint> &pts1, const std::vector<cv::KeyPoint> &pts2, std::vector<cv::Mat> &ret_E, std::vector<cv::Mat> &ret_P, std::vector<int> &ret_inliers)
{
    assert(pts1.size() >= 5);
    ...
    cv::Mat F(pts1.size(), 9, CV_64F);

    for(int i=0; i < pts1.size(); i++) {
        float x1 = pts1[i].pt.x;
        float y1 = pts1[i].pt.y;

        float x2 = pts2[i].pt.x;
        float y2 = pts2[i].pt.y;
    ...
    for(size_t i=0; i < solutions.size(); i++) {
        ...
        x1.at<double>(0,0) = pts1[0].pt.x;
        x1.at<double>(1,0) = pts1[0].pt.y;
        x1.at<double>(2,0) = 1.0;

        x2.at<double>(0,0) = pts2[0].pt.x;
        x2.at<double>(1,0) = pts2[0].pt.y;
        ...
        for(int j=0; j < 4; j++) {
          pt3d = TriangulatePoint(pts1[0].pt, pts2[0].pt, P_ref, P[j]);
            ...
            if(depth1 > 0 && depth2 > 0){
                ...
                for(int k=1; k < (int)pts1.size(); k++) {
                    pt3d = TriangulatePoint(pts1[k].pt, pts2[k].pt, P_ref, P[j]);
